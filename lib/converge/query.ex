defmodule Converge.Query do
  @moduledoc """
  Converge Query Language (LQL) - SQL-like queries for CRDT data.
  ...
  ## Quick Start
      ...
      {:ok, results} = Converge.Query.run("SELECT * FROM users WHERE active = true", data)
      # => [%{name: "Alice", age: 30, active: true, _key: "user1"}]

      {:ok, count} = Converge.Query.run("SELECT count(*) FROM users", data)
      # => 2
  ...
  alias Converge.Query.{Parser, Executor}
  ...
  ## Examples
      ...
      Converge.Query.run("SELECT * FROM users", %{"users" => users_map})
      Converge.Query.run("SELECT name FROM users WHERE age > 21", data)
      Converge.Query.run("SELECT count(*) FROM events", data)
  """
