defmodule Converge.Storage.Snapshot do
  @moduledoc """
  Snapshot management for Converge.
  ...
  @impl true
  def init(opts) do
    data_dir = Keyword.get(opts, :data_dir, Converge.Config.data_dir())
    interval_ms = Keyword.get(opts, :interval_ms, Converge.Config.get(:snapshot_interval_ms))
  ...
  defp create_snapshot(data_dir) do
    ...
    data =
      if Process.whereis(Converge.Storage.Store) do
        namespaces = Converge.Storage.Store.list_namespaces()

        Enum.reduce(namespaces, %{}, fn ns, acc ->
          keys = Converge.Storage.Store.list_keys(ns)

          entries =
            Enum.map(keys, fn key ->
              {key, Converge.Storage.Store.get(ns, key)}
            end)
            |> Map.new()
      ...
      {:ok, content} ->
        ...
        if Process.whereis(Converge.Storage.Store) do
          Enum.each(data, fn {namespace, entries} ->
            Converge.Storage.Store.create_namespace(namespace)

            Enum.each(entries, fn {key, value} ->
              Converge.Storage.Store.put(namespace, key, value)
            end)
          end)
        end
