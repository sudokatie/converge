defmodule Converge.Storage.Store do
  @moduledoc """
  ETS/DETS storage backend for Converge CRDTs.
  ...
  defp ensure_namespace(state, namespace) do
    if Map.has_key?(state.tables, namespace) do
      ...
      # Create ETS table
      table = :ets.new(:"converge_#{namespace}", [:set, :public])

      # Open DETS file
      dets_path = dets_path(state.data_dir, namespace)

      {:ok, dets} =
        :dets.open_file(:"converge_dets_#{namespace}",
