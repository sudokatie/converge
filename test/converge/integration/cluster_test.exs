defmodule Converge.Integration.ClusterTest do
  @moduledoc """
  ...
  alias Converge.Sync.Membership

  setup do
    # Create isolated test environment
    tmp_dir = System.tmp_dir!() |> Path.join("converge_integration_#{:rand.uniform(100_000)}")
  ...
  describe "persistence" do
    test "data persists to DETS and can be accessed", %{data_dir: _data_dir} do
      # Write data
      Converge.counter_inc("persist_test", "counter", 42)
      Converge.set_add("persist_test", "tags", "elixir")

      # Verify data is there
      assert Converge.counter_value("persist_test", "counter") == 42
      assert Converge.set_contains?("persist_test", "tags", "elixir")
    ...
    test "multiple operations accumulate correctly" do
      ...
      for i <- 1..10 do
        Converge.counter_inc("session", "events")
        Converge.set_add("session", "users", "user_#{i}")
      end

      assert Converge.counter_value("session", "events") == 10
      assert length(Converge.set_members("session", "users")) == 10
    end
  end
  ...
  describe "api integration" do
    test "full workflow: counter, set, map operations" do
      # Counter
      Converge.counter_inc("app", "visits", 100)
      Converge.counter_dec("app", "visits", 10)
      assert Converge.counter_value("app", "visits") == 90

      # Set
      Converge.set_add("app", "features", "dark-mode")
      Converge.set_add("app", "features", "notifications")
      assert length(Converge.set_members("app", "features")) == 2

      # Map
      Converge.map_put("app", "config", "theme", "dark")
      Converge.map_put("app", "config", "lang", "en")
      assert Converge.map_get("app", "config", "theme") == "dark"
      assert length(Converge.map_keys("app", "config")) == 2
    end

    test "namespace isolation works" do
      Converge.counter_inc("ns1", "key", 10)
      Converge.counter_inc("ns2", "key", 20)

      assert Converge.counter_value("ns1", "key") == 10
      assert Converge.counter_value("ns2", "key") == 20
    end
  end
  ...
  describe "persistence across restarts" do
    test "data survives store restart", %{data_dir: data_dir} do
      # Write data
      Converge.counter_inc("restart_test", "counter", 42)
      ...
      Converge.create_namespace("restart_test")
      assert Converge.counter_value("restart_test", "counter") == 42
    end
    ...
    test "multiple namespaces persist across restart", %{data_dir: data_dir} do
      ...
      Converge.create_namespace("ns3")

      # All data should persist
      assert Converge.counter_value("ns1", "key") == 10
      assert Converge.counter_value("ns2", "key") == 20
      assert Converge.set_contains?("ns3", "items", "a")
    end
  end
end
