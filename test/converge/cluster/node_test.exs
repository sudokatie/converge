defmodule Converge.Cluster.NodeTest do
  use ExUnit.Case

  alias Converge.Cluster.Node

  setup do
    tmp_dir = System.tmp_dir!() |> Path.join("converge_node_test_#{:rand.uniform(100_000)}")
  ...
  describe "explicit node_id option" do
    test "uses provided node_id instead of generating", %{data_dir: _data_dir} do
      ...
      tmp = System.tmp_dir!() |> Path.join("converge_explicit_#{:rand.uniform(100_000)}")
      File.mkdir_p!(tmp)
