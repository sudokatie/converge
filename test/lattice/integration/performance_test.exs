defmodule Lattice.Integration.PerformanceTest do
  @moduledoc """
  ...
  setup do
    tmp_dir = System.tmp_dir!() |> Path.join("lattice_perf_#{:rand.uniform(100_000)}")
  ...
  describe "large dataset sync performance" do
    ...
    test "store handles high write throughput" do
      ...
      {write_time_us, _} =
        :timer.tc(fn ->
          for i <- 1..@large_dataset_size do
            Lattice.counter_inc("perf_ns", "counter_#{i}")
          end
        end)
      ...
      for idx <- 1..10 do
        assert Lattice.counter_value("perf_ns", "counter_#{idx}") == 1
      end
    end
    ...
    test "concurrent read/write performance" do
      # Pre-populate
      for i <- 1..100 do
        Lattice.counter_inc("concurrent_ns", "key_#{i}", i)
      end
      ...
      tasks =
        for _i <- 1..10 do
          Task.async(fn ->
            for j <- 1..100 do
              if rem(j, 2) == 0 do
                Lattice.counter_inc("concurrent_ns", "key_#{rem(j, 100) + 1}")
              else
                Lattice.counter_value("concurrent_ns", "key_#{rem(j, 100) + 1}")
              end
            end
          end)
        end
