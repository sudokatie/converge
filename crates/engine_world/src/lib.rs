...
pub use environment::{
    ...
    FieldChannel, FieldSimConfig, FlowConfig, FlowLink, FluidCell, FluidDelta, FluidKind,
    FluidLayer, FluidResistanceMap, FluidSample, FluidTransportConfig, FluidTransportResult,
    GravityModel, GravityProfile, HazardCell, HazardCellDelta, HazardDeltaJournal,
    HazardDeltaRecord, HazardKind, HazardLayer, HazardSimulator, HazardSnapshot,
    HeatTransferConfig, LayerNeighborCounts, LoadConfig, MAX_GRAVITY_MAGNITUDE,
    MIN_GRAVITY_MAGNITUDE, MaterialCategory, MaterialId, MaterialProperties, MaterialRegistry,
    MaterialRegistryError, NodeRole, PressureConfig, PressureEqualizationPlan,
    PressureEqualizationStep, PressureMap, ProfileError, ProfileId, ProfileRegistry,
    PropagationConfig, PropagationResult, Resistance, RuleBundle, RuleOverrides, STANDARD_GRAVITY,
    SimStepResult, SimulationTickResult, SparseFluidConfig, SparseFluidEntry,
    SparseFluidFingerprint, SparseFluidPos, SparseFluidRegion, SparseFluidResult,
    SparseFluidSummary, SparseFluidValidation, SpreadConfig, StabilityConfig, StrengthMap,
    StructuralBoundary, StructuralCell, StructuralConfig, StructuralDelta, StructuralEvent,
    StructuralEventKind, StructuralResult, SupportKind, SupportPropagationConfig, TickStats,
    TransitionRules, VectorAdvectionConfig, VectorChannelData, VectorDecayConfig,
    VectorFieldChannel, VectorFieldSimConfig, VectorSmoothingConfig, WorldRuleProfile,
    apply_chunk_delta, apply_conduit_deltas, apply_equalization, apply_flows, apply_fluid_deltas,
    apply_snapshot, apply_structural_deltas, cell_from_material, check_decompression,
    compute_flow_links, detect_cavein, distribute_load, find_boundary_cells, find_networks,
    hazard_integration, layer_from_material, network_step, plan_pressure_equalization,
    propagate_support, simulate_chunk_tick, sparse_fluid_step, structural_step, transport_step,
};
