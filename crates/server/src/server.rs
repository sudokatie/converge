...
impl LatticeServer {
    ...
    pub fn run(&mut self) {
        ...
        while self.running {
            ...
            if accumulator < TICK_DURATION {
                let sleep_time = TICK_DURATION.saturating_sub(accumulator);
    ...
    fn tick(&mut self) {
        ...
        for event in events {
            match event {
                ...
                engine_network::transport::server::ServerNetEvent::ClientDisconnected(
                    client_id,
                ) => {
    ...
    #[expect(clippy::unused_self, reason = "placeholder for world simulation")]
    ...
    fn broadcast_snapshots(&mut self) {
        ...
        let player_snapshots: Vec<_> = self
            .players
            .values()
            .map(
                |player| engine_network::protocol::server_message::EntitySnapshot {
                    id: player.entity_id,
        ...
        for (&client_id, player) in &self.players {
            ...
            let entities: Vec<_> = player_snapshots
                .iter()
    ...
    #[expect(dead_code, reason = "public API for external callers")]
...
mod tests {
    ...
    fn player_state_initialization() {
        ...
        assert!((player.position.y - 64.0).abs() < f32::EPSILON);
