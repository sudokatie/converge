...
impl GameServer {
    ...
    pub fn new(port: u16) -> Result<Self, ServerError> {
        ...
        let protocol_id = 0x4C41_5454; // "LATT" in hex
        ...
        while let Some(event) = self.server.get_event() {
            match event {
                ServerEvent::ClientConnected { client_id } => {
                    ...
                    self.client_names
                        .insert(client_id, format!("Player{client_id}"));
...
fn message_channel(message: &ServerMessage) -> u8 {
    match message {
        ServerMessage::Snapshot(_) | ServerMessage::EntityUpdate { .. } => channels::UNRELIABLE,
