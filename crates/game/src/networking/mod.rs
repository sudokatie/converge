...
use engine_network::{
    ClientMessage, GameClient, ServerMessage, WorldSnapshot, protocol::InputState,
};
use glam::Vec3;
...
impl NetworkClient {
    ...
    pub fn update(&mut self, dt: Duration) {
        ...
        } else if !client.is_connecting() && self.state != NetworkState::Disconnected {
            self.state = NetworkState::Disconnected;
            self.pending_events.push(NetworkEvent::Disconnected);
        }
    ...
    fn process_message(&mut self, message: ServerMessage) {
        match message {
            ...
            ServerMessage::PlayerJoined { id, name } => {
                self.pending_events
                    .push(NetworkEvent::PlayerJoined { id, name });
            }
            ...
            ServerMessage::ChatReceive { sender, message } => {
                self.pending_events
                    .push(NetworkEvent::ChatReceived { sender, message });
            }
    ...
    pub fn rtt_ms(&self) -> f64 {
        self.client.as_ref().map_or(0.0, GameClient::rtt_ms)
    }

    /// Get packet loss percentage.
    pub fn packet_loss(&self) -> f64 {
        self.client.as_ref().map_or(0.0, GameClient::packet_loss)
    }
}
